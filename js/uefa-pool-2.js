...
epool.maxPoint = 0;
epool.ARRank = "";     // TO BE UPDATED
epool.CHRank = "";     // TO BE UPDATED
epool.chartData = [];
epool.chartNode = {
    name: "",
    point: 0
...
epool.initElementsEvent = function () {
    ...
    $('#btnSave').on('click', function () {
        ...
        if (name !== "" && epool.isEntryExist(name) !== true && epool.entries.length < 4) {
            ...
            if (epool.entry.t8 === epool.entry.t9) {
                M.toast({ html: "Arsenal's place can't be the same as Chelsea's. " }, 3000);
                return;
            }
            ...
            if (name === "")
                M.toast({ html: 'A name of the entry is required' }, 3000);
            else if (epool.isEntryExist(name) === true)
                M.toast({ html: 'An entry with same name is alreay exist' }, 3000);
            else if (epool.entries.length >= 4) 
                M.toast({ html: "Everyone can't have more than 4 entries." }, 3000);
        }
    });
    ...
    $('input[name="group1"]').on('click', function () {
        epool.entry.r1t1 = $(this).val();
        console.log(epool.entry);
        epool.round2Layout();
    });
    $('input[name="group2"]').on('click', function () {
        epool.entry.r1t2 = $(this).val();
        console.log(epool.entry);
        epool.round2Layout();
    });
    $('input[name="group3"]').on('click', function () {
        epool.entry.r1t3 = $(this).val();
        console.log(epool.entry);
        epool.round2Layout();
    });
    $('input[name="group4"]').on('click', function () {
        epool.entry.r1t4 = $(this).val();
        console.log(epool.entry);
        epool.round2Layout();
    });
}
...
epool.round2Layout = function () {
    let sh = '';

    if (epool.entry.r1t1 !== "" && epool.entry.r1t2 !== "" && epool.entry.r1t3 !== "" && epool.entry.r1t4 !== "") {
        $('#round2').empty();

        sh += '<div class="row center-align rowCatA" style="width: 100%">';
        sh += '<div class="col s3 dvLabel">';
        sh += '<label>';
        sh += '<input name="group5" type="radio" value="' + epool.entry.r1t1 + '" />';
        sh += '<span>' + teamsObjects[epool.entry.r1t1] + '</span>';
        sh += '</label>';
        sh += '</div>';
        sh += '<div class="col s3 dvLabel">game 1 (winner)</div>';
        sh += '<div class="col s4 dvSelect">';
        sh += '<select id="sel-catA" class="browser-default">';
        sh += '<option value="' + epool.entry.r1t1 + '">' + teamsObjects[epool.entry.r1t1] + '</option>';
        sh += '<option value="' + epool.entry.r1t2 + '">' + teamsObjects[epool.entry.r1t2] + '</option>';
        sh += '<option value="ZZ">Tie</option>';
        sh += '</select>';
        sh += '</div>';
        sh += '<div class="col s2 dvPoint">';
        sh += '<input value="0" id="catAPoint" type="text" readonly style="text-align: center">';
        sh += '</div>';
        sh += '</div>';

        sh += '<div class="row center-align rowCatB" style="width: 100%; border-bottom: 1px dotted">';
        sh += '<div class="col s3 dvLabel">';
        sh += '<label>';
        sh += '<input name="group5" type="radio" value="' + epool.entry.r1t2 + '" />';
        sh += '<span>' + teamsObjects[epool.entry.r1t2] + '</span>';
        sh += '</label>';
        sh += '</div>';
        sh += '<div class="col s3 dvLabel">game 2 (winner)</div>';
        sh += '<div class="col s4 dvSelect">';
        sh += '<select id="sel-catB" class="browser-default">';
        sh += '<option value="' + epool.entry.r1t1 + '">' + teamsObjects[epool.entry.r1t1] + '</option>';
        sh += '<option value="' + epool.entry.r1t2 + '">' + teamsObjects[epool.entry.r1t2] + '</option>';
        sh += '<option value="ZZ">Tie</option>';
        sh += '</select>';
        sh += '</div>';
        sh += '<div class="col s2 dvPoint">';
        sh += '<input value="0" id="catBPoint" type="text" readonly style="text-align: center">';
        sh += '</div>';
        sh += '</div>';

        sh += '<div class="row center-align rowCatC" style="width: 100%">';
        sh += '<div class="col s3 dvLabel">';
        sh += '<label>';
        sh += '<input name="group6" type="radio" value="' + epool.entry.r1t3 + '" />';
        sh += '<span>' + teamsObjects[epool.entry.r1t3] + '</span>';
        sh += '</label>';
        sh += '</div>';
        sh += '<div class="col s3 dvLabel">game 1 (winner)</div>';
        sh += '<div class="col s4 dvSelect">';
        sh += '<select id="sel-catC" class="browser-default">';
        sh += '<option value="' + epool.entry.r1t3 + '">' + teamsObjects[epool.entry.r1t3] + '</option>';
        sh += '<option value="' + epool.entry.r1t4 + '">' + teamsObjects[epool.entry.r1t4] + '</option>';
        sh += '<option value="ZZ">Tie</option>';
        sh += '</select>';
        sh += '</div>';
        sh += '<div class="col s2 dvPoint">';
        sh += '<input value="0" id="catCPoint" type="text" readonly style="text-align: center">';
        sh += '</div>';
        sh += '</div>';

        sh += '<div class="row center-align rowCatD" style="width: 100%">';
        sh += '<div class="col s3 dvLabel">';
        sh += '<label>';
        sh += '<input name="group6" type="radio" value="' + epool.entry.r1t4 + '" />';
        sh += '<span>' + teamsObjects[epool.entry.r1t4] + '</span>';
        sh += '</label>';
        sh += '</div>';
        sh += '<div class="col s3 dvLabel">game 2 (winner)</div>';
        sh += '<div class="col s4 dvSelect">';
        sh += '<select id="sel-catD" class="browser-default">';
        sh += '<option value="' + epool.entry.r1t3 + '">' + teamsObjects[epool.entry.r1t3] + '</option>';
        sh += '<option value="' + epool.entry.r1t4 + '">' + teamsObjects[epool.entry.r1t4] + '</option>';
        sh += '<option value="ZZ">Tie</option>';
        sh += '</select>';
        sh += '</div>';
        sh += '<div class="col s2 dvPoint">';
        sh += '<input value="0" id="catDPoint" type="text" readonly style="text-align: center">';
        sh += '</div>';
        sh += '</div>';
    ...
    $('input[name="group5"]').on('click', function () {
        epool.entry.r2t1 = $(this).val();
        console.log(epool.entry);
        epool.round3Layout();
    });
    $('input[name="group6"]').on('click', function () {
        epool.entry.r2t2 = $(this).val();
        console.log(epool.entry);
        epool.round3Layout();
    });
}

epool.round3Layout = function () {
    let sh = '';

    if (epool.entry.r2t1 !== "" && epool.entry.r2t2 !== "") {
        $('#round3').empty();

        sh += '<div class="row center-align rowCatF" style="width: 100%">';
        sh += '<div class="col s3 dvLabel">';
        sh += '<b>' + teamsObjects[epool.entry.r2t1] + '</b>&nbsp;&nbsp;vs&nbsp;&nbsp;<b>' + teamsObjects[epool.entry.r2t2] + '</b>';
        sh += '</div>';
        sh += '<div class="col s3 dvLabel">game (winner)</div>';
        sh += '<div class="col s4 dvSelect">';
        sh += '<select id="sel-catF" class="browser-default">';
        sh += '<option value="' + epool.entry.r2t1 + '">' + teamsObjects[epool.entry.r2t1] + '</option>';
        sh += '<option value="' + epool.entry.r2t2 + '">' + teamsObjects[epool.entry.r2t2] + '</option>';
        sh += '<option value="ZZ">Tie</option>';
        sh += '</select>';
        sh += '</div>';
        sh += '<div class="col s2 dvPoint">';
        sh += '<input value="0" id="catFPoint" type="text" readonly style="text-align: center">';
        sh += '</div>';
        sh += '</div>';
    ...
    $('input[name="group7"]').on('click', function () {
        epool.entry.r3 = $(this).val();
        console.log(epool.entry);
    });
}
...
epool.initEntryLoading = function () {
    ...
    if (localStorage.getItem("entriesJson")) {
        ...
        //epool.afterEntriesLoaded(entriesObj); //Remove comment //
        return epool.maxPoint;
    }
    else {
